...
function parseStilZeitenFromGrid(rows: string[][]): StilZeiten[] {
    ...
    for (let row of rows) {
        ...
        // process zeit
        if (group !== undefined) {
            let schwimmerZeit = parseSchwimmerZeitRow(group.stil, row);
            if (schwimmerZeit) {
...
function parseSchwimmerZeitRow(
    stil: string,
    row: string[],
): SchwimmerZeit | undefined {
...
function groupBySchwimmer(stilZeitenList: StilZeiten[]): Schwimmer[] {
    ...
    for (let stilZeiten of stilZeitenList) {
        for (let zeit of stilZeiten.zeiten) {
    ...
    return Array.from(schwimmerList.entries())
        .map(([key, value]) => ({name: key, zeitenSeconds: value}));
}
